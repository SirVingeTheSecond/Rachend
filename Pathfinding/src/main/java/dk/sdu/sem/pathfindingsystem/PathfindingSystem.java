...
public class PathfindingSystem implements IUpdate, IGUIUpdate {
	...
	private static List<Vector2D> findPath(Vector2D start, Vector2D target, Function<Vector2D, Boolean> sampleGrid) {
		PriorityQueue<PathNode> unexploredSet = new PriorityQueue<>(Comparator.comparingInt(n -> n.fCost));
		...
		while (!unexploredSet.isEmpty()) {
			...
			for (Vector2D direction : Vector2D.DIRECTIONS) {
				...
				int preliminaryGCost = current.gCost + (int) Vector2D.euclidean_distance(current.position, neighborPos);
